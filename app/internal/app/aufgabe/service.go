...
func NewService(store portaufgabe.Store, projects *appprojekt.Service, agents *appagent.Service) *Service {
	return &Service{store: store, projects: projects, agents: agents}
}
...
func (s *Service) Create(ctx context.Context, organizationID string, input CreateInput) (domainaufgabe.Task, error) {
	if s == nil || s.store == nil || s.projects == nil || s.agents == nil {
		return domainaufgabe.Task{}, ErrAccessDenied
	}

	task, err := s.prepare(ctx, organizationID, input)
	if err != nil {
...
func (s *Service) prepare(ctx context.Context, organizationID string, input CreateInput) (domainaufgabe.Task, error) {
	...
	if err := s.checkAssignee(ctx, organizationID, task.AssigneeID); err != nil {
		return domainaufgabe.Task{}, err
	}
...
func (s *Service) checkAssignee(ctx context.Context, organizationID, assigneeID string) error {
