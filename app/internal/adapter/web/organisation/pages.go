...
func (h *Handler) pageForm(w http.ResponseWriter, r *http.Request) {
	...
	h.render(w, r, http.StatusOK, h.formData("", "", ""))
}
...
func (h *Handler) pageCreate(w http.ResponseWriter, r *http.Request) {
	...
	if err := r.ParseForm(); err != nil {
		h.render(w, r, http.StatusBadRequest, h.formData("", "", "Formulardaten konnten nicht gelesen werden."))
		return
	}
...
func (h *Handler) createFromForm(w http.ResponseWriter, r *http.Request) {
	for key := range r.PostForm {
		if key != "name" && key != "description" {
			h.pageFailure(w, r, http.StatusBadRequest, "Unbekanntes Formularfeld")
			return
		}
	}
	...
	if err != nil {
		h.formError(w, r, err, r.PostFormValue("name"), r.PostFormValue("description"))
		return
	}
...
func (h *Handler) formError(w http.ResponseWriter, r *http.Request, err error, name, description string) {
	if errors.Is(err, apporganisation.ErrNameRequired) {
		h.render(w, r, http.StatusUnprocessableEntity, h.formData(name, description, "Bitte geben Sie einen Namen ein."))
		return
	}

	if errors.Is(err, apporganisation.ErrNameConflict) {
		h.render(w, r, http.StatusConflict, h.formData(name, description, "Dieser Name ist bereits vergeben."))
		return
	}
