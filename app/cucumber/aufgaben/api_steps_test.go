...
func (s *Suite) pauseAgentInStore(name, org string) error {
	...
	db, err := sqlite.OpenWithMigrations(ctx, s.database, sqlite.RunMigration(2), sqlite.OrganizationMigration(), sqlite.GoalMigration(), sqlite.AgentMigration(), sqlite.ProjectMigration(), sqlite.DataScopeMigration(), sqlite.CodexProfileMigration(), sqlite.TaskMigration())
	if err != nil {
		return err
	}
