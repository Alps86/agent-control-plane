...
const directory = await mkdtemp(join(tmpdir(), 'acp-scope-chrome-'))
...
const snapshot = `(() => ({
  ...
  status: document.querySelector('main [role=status]')?.textContent?.replace(/\\s+/g, ' ').trim() || ''
}))()`
...
async function page(wait, previous) {
  for (let attempt = 0; attempt < 100; attempt++) {
    ...
    const changed = !previous || value.epoch !== previous.epoch || value.url !== previous.url || value.text !== previous.text || value.status !== previous.status
...
async function command(input) {
  ...
  if (input.check) {
    const checked = await evaluate(`(() => { const element = document.querySelector(${JSON.stringify(input.check.selector)}); if (!element) return false; element.checked = ${input.check.checked}; element.dispatchEvent(new Event('change', {bubbles:true})); return true })()`)
    if (!checked) throw new Error(`Checkbox fehlt: ${input.check.selector}`)
  }
  if (input.submit) {
  ...
  return { ok: true, page: await page(input.wait, input.check ? undefined : previous) }
}
