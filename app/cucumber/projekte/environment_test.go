...
func NewSuite(t *testing.T) *Suite {
	...
	return &Suite{t: t, client: client, organizations: map[string]string{}, goals: map[string]string{}, projects: map[string]string{}}
}
...
func (s *Suite) freshServer() error {
	s.stopServer()
	s.database = filepath.Join(s.t.TempDir(), "projekte.sqlite")
...
func (s *Suite) launch() error {
	file, err := os.CreateTemp(s.t.TempDir(), "projekte-server-")
	if err != nil {
		return err
	}
...
func (s *Suite) afterScenario(ctx context.Context, _ *godog.Scenario, _ error) (context.Context, error) {
	...
	s.unknownBody = nil
	s.rejectedAddr, s.rejectedLog, s.rejectedErr = "", "", nil
	s.routePages = nil
...
func (s *Suite) requestTo(method, target, body string, headers http.Header, host string) error {
	req, err := s.newRequest(method, target, body, headers, host)
	if err != nil {
		return err
	}
...
func (s *Suite) newRequest(method, target, body string, headers http.Header, host string) (*http.Request, error) {
